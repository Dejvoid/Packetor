...
void NetScanner::scan_mac_passive(PcapLiveDevice* device, int wait_time) {
    ...
    if (device->open()) {
        device->startCaptureBlockingMode(on_packet_arrival, &mac_devs_, wait_time);
    }
    else 
...
void NetScanner::scan_ipv4_passive(PcapLiveDevice* device, int wait_time) {
    ...
    if (device->open()) {
        device->startCaptureBlockingMode(on_packet_arrival_ipv4, &ipv4_devs_, wait_time);
    }
    else 
        std::cout << "Device in use: " << device->getName() << std::endl;
    std::cout << "Scan done!" << std::endl;
...
void NetScanner::scan_ipv6_passive(PcapLiveDevice* device, int wait_time) {
    ...
    if (device->open()) {
        device->startCaptureBlockingMode(on_packet_arrival_ipv6, &ipv6_devs_, wait_time);
    }
    else 
        std::cout << "Device in use: " << device->getName() << std::endl;
    std::cout << "Scan done!" << std::endl;
