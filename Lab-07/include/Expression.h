...
class Parser {
    private:
        static OperationNode* prepare_op_node(char op, Node* l, Node* r);
        static void process_operator(char op, std::stack<Node*>& operands);
        inline static bool is_operator(char o);
        inline static bool is_higher_precedence(char op);
        static int process_number(std::string::const_iterator& it, const std::string& input);
    public:
        static Node* parse_from_string(const std::string& input);
};
