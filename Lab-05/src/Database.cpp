...
bool Database::process_csv_line(const std::string& line, Movie& m){
    char splitter = ';';
    char list_splitter = ',';
    bool ret_value = true;
    ...
    m = Movie(std::move(name), year, std::move(genre), rating, std::move(actors));
    return ret_value;
}

bool Database::import(std::istream& is, std::vector<Movie>& db){
    bool ret_value = true;
...
bool Database::import(const std::string& filename, std::vector<Movie>& db){
